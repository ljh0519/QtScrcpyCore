...
class Decoder : public IDecoder
{
    Q_OBJECT
public:
    Decoder(std::function<void(int width, int height, uint8_t* dataY, uint8_t* dataU, uint8_t* dataV, int linesizeY, int linesizeU, int linesizeV)> onFrame,
            AVCodecID codecId, QObject *parent = Q_NULLPTR);
    virtual ~Decoder();
